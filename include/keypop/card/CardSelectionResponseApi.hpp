...
/**
 * Data from the start-up phase with the card: selection data and any additional
 * responses.
 *
 * <p>Includes the status of the selection itself and the responses to any
...
public:
     ...
     * standard), but other devices can have their own definition, including for
     * example elements from the anti-collision stage of the ISO14443 protocol
     * (ATQA, ATQB, ATS, SAK, etc) or any proprietary definitions.
     *
     * <p>These data being variable from one reader to another, they are defined
     * here in string format which can be either a hexadecimal string or any
    ...
    /**
     * Gets the calypsonet::terminal::card::ApduResponseApi received from the
     * card in response to
     ...
     * @since 1.0.0
     */
    virtual const std::shared_ptr<ApduResponseApi>
     ...
     */
    friend std::ostream&
    operator<<(std::ostream& os, const CardSelectionResponseApi& csr) {
        os << "CARD_SELECTION_RESPONSE_API: {"
           << "HAS_MATCHED: " << csr.hasMatched() << ", "
           << "POWER_ON_DATA: " << csr.getPowerOnData() << ", "
           << "CARD_RESPONSE: " << csr.getCardResponse() << "}";
    ...
    friend std::ostream&
    operator<<(
        std::ostream& os, const std::shared_ptr<CardSelectionResponseApi> csr) {
        if (csr) {
            os << *csr.get();
        } else {
            os << "CARD_SELECTION_RESPONSE_API: null";
        }
